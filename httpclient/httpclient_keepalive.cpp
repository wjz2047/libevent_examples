...
int main(int argc, char **argv)
{
    ...
    conns = (struct evhttp_connection **)calloc(conn_num,
    ...
    for (int i = 0; i < conn_num; i++) {
        conns[i] = evhttp_connection_base_new(main_base, NULL, host, port);
    }
...
void cmd_msg_cb(int fd, short events, void *arg) {
    ...
    for (int i = 0; i < conn_num; i++) {
        for (int j = 0; j < req_num; j++) {
            ...
            evhttp_make_request(conns[i], req, EVHTTP_REQ_POST, uri);
        }
    }
    req_index += req_num;
}
